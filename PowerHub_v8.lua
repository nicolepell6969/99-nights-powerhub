...
Sl.Name = "PowerHub99v8"; Sl.ResetOnSpawn = false
...
function Window:New(opts)
    ...
    local ver = Lbl("v8",11,UI.Theme.sub); ver.Size=UDim2.new(0.2,0,1,0); ver.Position=UDim2.new(0.62,0,0,0); ver.Parent=header
...
-- remote helper (confirmed names from dump v5)
local function RE(name) return RemoteEvents:FindFirstChild(name) end
local function fire(name, ...)
    local r = RE(name); if not r then return end
    local args = {...}
...
local state = { godmode=false, chop=false, campfire=false, antiafk=false, killaura=false, stronghold=false, bringchildren=false, revealmap=false, scrapeToggle=false, brTools=false, brConsumable=false, brWeapon=false }
...
-- AUTO BRING CHILDREN (Save Kids) - 4-phase state machine (from dump v5 mechanics)
-- FIND -> launch SelectKidTask -> COLLECT: tp to Lost Child -> BAG: RequestBagStoreItem ->
-- CAMP: tp camp & RequestBagDropItem -> FIND. Kid absent-in-workspace => use SelectKidTask to spawn.
local KidPhase = "idle"
local KidCampPos
local function findLostChild()
    ...
    for _,m in ipairs(charsFolder:GetChildren()) do
        if m.Name == "Lost Child" or m.Name:find("Lost Child") then return m end
    end
    return nil
end
local function findCampPos()
    -- detect the camp: look for Campground / firepit landmark
    local mapM = Workspace:FindFirstChild("Map")
    if mapM then
        for _,v in ipairs(mapM:GetDescendants()) do
            local nm = v.Name:lower()
            if v:IsA("BasePart") and (nm:find("camp") or nm:find("firepit") or nm:find("campground")) then
                return v.Position
            end
        end
    end
    local f = findCampfire(); if f then return f.Position end
    return nil
end
local function setBringChildren(on)
    ...
    conns.bringchildren = RunService.Heartbeat:Connect(function()
        ...
        -- FIND: no child present -> ask server to spawn next missing child
        if KidPhase == "idle" then
            local child = findLostChild()
            if child then
                KidPhase = "collect"
            else
                fire("SelectKidTask")
        ...
        if KidPhase == "collect" then
            local child = findLostChild()
            if child then
                local tr = child:FindFirstChild("HumanoidRootPart")
                if tr then
                    -- approach child
                    local d = (hrp.Position - tr.Position).Magnitude
                    if d > 6 then
                        hrp.CFrame = tr.CFrame + Vector3.new(0,0,4)
                    else
                        -- near child: store into bag
                        invoke("RequestBagStoreItem", child)
                        KidPhase = "bag"
                        task.wait(0.5)
                    end
                end
            else
                KidPhase = "idle" -- child vanished (maybe collected); re-ask
            end
            return
        end
        if KidPhase == "bag" then
            -- child should now be in bag; go to camp
            KidCampPos = KidCampPos or findCampPos()
            if KidCampPos then
                hrp.CFrame = CFrame.new(KidCampPos + Vector3.new(0,3,0))
                fire("RequestBagDropItem")
                task.wait(0.4)
                KidPhase = "idle"
            else
                KidPhase = "idle"
...
-- AUTO REVEAL MAP (fire map update remotes, confirmed from dump v5)
local function setRevealMap(on)
    ...
    conns.revealmap = RunService.Heartbeat:Connect(function()
        ...
        fire("UpdateMapCells")
        fire("FoundLandmark")
        -- RequestServerMapProgress is a RemoteFunction; invoke for progress
        invoke("RequestServerMapProgress")
        pcall(function() fire("MapCellsDiscovered") end)
    end)
end
...
local function bringFiltered(nameSet, bringToolsFlag)
    -- For items in Workspace.Items: move BaseParts to target.
    -- For tools: bring items from inventory/ReplicatedStorage Tools defs via bag remote.
    ...
    if folder then
        for _,item in ipairs(folder:GetChildren()) do
            ...
            if item:IsA("BasePart") then
                ...
                if nameOk then
                    local targetPos = (TargetChoice == "camp" and destPos) or (hrp.Position + hrp.CFrame.RightVector*3 + Vector3.new(0,1,0))
...
print("\n======================================")
print(" 🔮 PowerHub v8 • 99 Nights in the Forest")
print("======================================")
...
print(" Remote-wired from data dump v5")
print("======================================\n")