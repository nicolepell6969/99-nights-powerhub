-- ============================================
--  99 NIGHTS IN THE FOREST - POWER HUB v6
--  Refined GUI: iOS toggles, draggable sliders,
--  smooth hover. Mob-targeting Kill Aura & Farm.
--  Motion: Fly, Noclip, Speed, Jump, Godmode,
--  Kill Aura, Bring Items, Auto Farm, ESP,
--  Fullbright, Teleport
-- ============================================
...
-- ============ STATE & CONNECTIONS ============
local state = { fly = false, noclip = false, godmode = false, killaura = false, bring = false, farm = false, espP = false, espI = false, fullbright = false }
...
local function isEnemyMob(character)
    -- Must be in the Characters folder (hostile mobs spawn there)
    local parent = character and character.Parent
    if not parent or parent.Name ~= "Characters" then return false end
    -- Exclude friendly/named NPCs / non-mobs by name
    local name = character.Name
    local friendly = {
        ["Tool Trader"]=true, ["Cultist King"]=true, ["GeneratorExtension"]=true,
        ["FlashlightFill"]=true, ["Lost Child"]=true,
    }
    if friendly[name] then return false end
    return true
end
...
local function setKillAura(on)
    ...
    conns.killaura = RunService.Heartbeat:Connect(function()
        ...
        for _, target in ipairs(charsFolder:GetChildren()) do
            if target:IsA("Model") and isEnemyMob(target) then
...
MiscTab:Label("")
MiscTab:Label("v6 • Mob bundle ditambahkan • Gunakan di server pribadi.", Color3.fromRGB(110, 110, 110))

print("\n======================================")
print(" 🔮 PowerHub v6 • 99 Nights in the Forest")
print("======================================")
print(" Motion: Fly, Noclip, Speed, Jump")
print(" Combat: Kill Aura, Godmode")
